...
from local_agent_runtime.connectors.xhs.context import is_suspect_detail_author_name
...
def _extract_platform_tags(*texts: str | None) -> list[str]:
    tags: list[str] = []
    for text in texts:
        if not text:
            continue
        tags.extend(re.findall(r"#[^\s#]+", text))
...
def normalize_xhs_detail_payload(
    ...
    *,
    ...
    api_note = raw_payload.get("api") if isinstance(raw_payload.get("api"), dict) else {}
    note = api_note or _find_note_payload(raw_payload, platform_content_id)
    ...
    if is_suspect_detail_author_name(author_name, upstream_author_name=upstream_author_name):
        author_name = upstream_author_name or None
        dom_fallback = {**dom_fallback, "suspect_author": True, "author_name": author_name}
    ...
    return DetailSnapshotInput(
        ...
        raw_payload={
            ...
            "platform_tags": _extract_platform_tags(
                _first(note.get("desc"), note.get("description"), note.get("content"), dom_fallback.get("body_text"))
            ),
        },
    )
