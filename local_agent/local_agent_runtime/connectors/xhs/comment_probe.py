...
from local_agent_runtime.connectors.xhs.comment_normalizer import normalize_xhs_comments
from local_agent_runtime.connectors.xhs.detail_probe import parse_json_like_script
from local_agent_runtime.connectors.xhs.context import build_xhs_note_url, context_from_url_and_raw, merge_xhs_context, url_has_xsec_context
...
def detect_comment_surface_status(*, url: str, body_text: str, comment_node_count: int) -> tuple[str, str | None, str | None]:
    url_lower = url.lower()
    if "error_code=300031" in url_lower or "暂时无法浏览" in url or "暂时无法浏览" in body_text:
        return "note_unavailable", "note_unavailable", "note page is unavailable in current session"
    ...
    if "评论" in body_text and any(marker in body_text for marker in ("评论已关闭", "无法评论", "作者已关闭评论")):
        return "comments_disabled", "comments_disabled", "comments are disabled or hidden on this note"
    ...
    if "login" in url_lower or any(marker in body_text for marker in LOGIN_REQUIRED_MARKERS):
        return "login_required", "login_required", "xhs login is required"
    if comment_node_count == 0 and "评论" not in body_text and "comment" not in body_text.lower():
        return "comment_surface_unavailable", "dom_structure_changed", "comment section not detected; DOM structure may have changed"
    return "ok", None, None
...
class XhsCommentProbe:
    async def fetch_comments_result(
        ...
        *,
        ...
        if not resolved_context.get("api_comment_ready"):
            return XhsCommentFetchResult(
                surface_status="missing_xsec_context",
                error_code="missing_xsec_context",
                message="xhs comment fetch requires note_id and xsec_token from search/homefeed URL context",
                diagnostics={
                    "url": canonical_url,
                    "platform_content_id": platform_content_id,
                    "platform_context": {k: v for k, v in resolved_context.items() if k != "xsec_token"},
                },
            )
        try:
            ...
            if api_comments or raw_comments == []:
                return XhsCommentFetchResult(
                    ...
                    diagnostics={**api_diagnostics, "api_page_count": api_diagnostics.get("page_count"), "source_path": "api", "platform_context": resolved_context},
                )
        ...
        for _ in range(8):
            ...
            if len(network_payloads) == last_payload_count:
                no_growth_count += 1
            else:
                no_growth_count = 0
                last_payload_count = len(network_payloads)
        ...
        if len(network_comments) >= limit:
            return XhsCommentFetchResult(
                ...
                diagnostics={"source": "network", "source_path": "network", "network_payload_count": len(network_payloads), "actual_scroll_count": actual_scroll_count},
            )
        ...
        if len(comments) >= limit:
            return XhsCommentFetchResult(
                ...
                diagnostics={"source": "network+state", "source_path": "network", "network_payload_count": len(network_payloads), "actual_scroll_count": actual_scroll_count},
            )
