...
from intelligence_engine.connectors.xhs.creator import XhsCreatorConnector
...
from intelligence_engine.connectors.xhs.search_probe import XhsSearchProbe
from intelligence_engine.domain.enums import ErrorCode, JobStatus, JobType, Platform, SessionStatus
...
class XhsJobExecutor:
    ...
    def _resolve_session_meta(self, meta: dict[str, Any]) -> dict[str, Any]:
        ...
        if profile_key and not resolved.get("user_data_dir") and not resolved.get("profile_ref"):
            ...
            from intelligence_engine.local_agent.chrome_launcher import resolve_profile_dir
    ...
    async def _run_search_collect(self, job: ClaimedJobPayload, page) -> JobExecutionResult:
        ...
        candidates, report = await XhsSearchProbe(keywords=keywords, max_items=max_items).collect(page)
    ...
    async def _run_detail(self, job: ClaimedJobPayload, page) -> JobExecutionResult:
        ...
        return JobExecutionResult(
            ...
            result_summary={"snapshot_id": ingestion.get("snapshot_id"), "comment_job_enqueued": ingestion.get("comment_job_enqueued"), "runtime": "local_agent_runtime_v1"},
        )
    ...
    async def _run_creator_monitor(self, job: ClaimedJobPayload, page) -> JobExecutionResult:
        ...
        return JobExecutionResult(
            ...
            result_summary={**ingestion, "runtime": "local_agent_runtime_v1"},
        )
...
class LocalAgentRuntime:
    ...
    async def _handle_login_sessions(self, agent_id: str) -> int:
        ...
        from intelligence_engine.local_agent.account_login_executor import AccountLoginExecutor, LoginSessionPayload
