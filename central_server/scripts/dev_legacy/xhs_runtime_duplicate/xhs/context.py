...
def merge_xhs_context(*contexts: dict[str, Any] | XhsNoteContext | None) -> dict[str, Any]:
    """Merge XHS request context.

    `note_id` is stable, while `xsec_token`/`xsec_source` come from discovery URLs
    and should prefer the latest non-empty value passed to this function.
    """
...
def prefer_richer_xhs_url(existing: str | None, candidate: str | None) -> str | None:
    ...
    # Prefer the latest URL when it carries a complete XHS request context.
